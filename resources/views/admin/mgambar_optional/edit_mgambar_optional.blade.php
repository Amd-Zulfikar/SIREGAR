...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <form method="POST" action="{{ route('update.mgambaroptional', $mgambaroptional->id) }}"
                                                ...
                                                <select name="mdata_id" class="form-control select2" style="width: 100%;"
                                                    ...
                                                    @foreach ($mdatas as $mdata)
                                                        <option value="{{ $mdata->id }}"
                                                            {{ $mdata->id == $mgambaroptional->mdata_id ? 'selected' : '' }}>
                                                            {{ $mdata->engine->name ?? '-' }} -
                                                            {{ $mdata->brand->name ?? '-' }} -
                                                            {{ $mdata->chassis->name ?? '-' }} -
                                                            {{ $mdata->vehicle->name ?? '-' }}
                                            ...
                                            <div class="form-group border p-3 rounded" style="background-color: #f7f9fc;">
                                                ...
                                                <div class="mb-2">
                                                    @if ($mgambaroptional->file_path && Storage::disk('public')->exists($mgambaroptional->file_path))
                                                        <img src="{{ Storage::url($mgambaroptional->file_path) }}"
                                                ...
                                                <input type="text" name="description"
                                                    ...
                                                    value="{{ old('description', $mgambaroptional->description) }}"
                                                    required>
                                            </div>
                                        </div>
                                    </div>
