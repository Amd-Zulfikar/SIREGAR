                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-left">
                            <a type="button" class="btn btn-block bg-gradient-primary"
                                onclick="location.href='{{ route('add.vehicle') }}'">Tambah Data</a>
                        </ol>
                    </div>
                    <div class="col-sm-6">
                                            ...
                                            @forelse($vehicles as $vehicle)
                                                ...
                                                <td>
                                                    <input type="checkbox" class="status-switch"
                                                        data-id="{{ $vehicle->id }}" name="status"
                                                        {{ $vehicle->status ? 'checked' : '' }} data-bootstrap-switch
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('switchChange.bootstrapSwitch', '.status-switch', function(event, state) {
                ...
                $.ajax({
                    url: "{{ url('admin/vehicle/action') }}/" + id,
                    method: "POST",
                    data: {
                        status: newStatus,
                        _token: "{{ csrf_token() }}"
